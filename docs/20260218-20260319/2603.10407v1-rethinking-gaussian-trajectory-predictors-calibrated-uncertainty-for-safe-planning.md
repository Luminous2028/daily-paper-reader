...
score: 8.0
evidence: 为安全自动驾驶导航和规划校准预测不确定性
tldr: 本研究针对自动驾驶中高斯轨迹预测器置信度不可靠的问题，提出了一种新型校准损失函数。该方法利用核密度估计（KDE）将预测分布的经验置信水平与卡方分布对齐，并结合均方误差优化预测精度。实验证明，该方法显著提升了多种先进预测器的不确定性估计可靠性，并在结合模型预测控制（MPC）的复杂场景导航中实现了更高的安全性与规划性能。
selection_source: fresh_fetch
motivation: 现有的高斯轨迹预测器因依赖负对数似然损失，常导致置信度估计失准，进而影响下游不确定性感知规划器的安全性。
method: 提出一种基于核密度估计的损失函数，通过将经验置信分布与卡方分布匹配来校准不确定性，并结合MSE确保均值预测精度。
result: 在真实轨迹数据集上的实验表明，该方法显著提高了多种先进预测器的置信度可靠性，并提升了避障规划的性能。
conclusion: 校准后的不确定性估计对于实现复杂环境下的安全自动导航至关重要，能有效平衡规划的安全性与保守性。
---
