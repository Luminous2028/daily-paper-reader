...
## 速览
**TLDR**：针对现有机器人系统将不确定性简化为单一标量而导致纠错效率低下的问题，本文提出 TRIAGE 框架。该框架将不确定性分解为偶然（观测噪声）和认知（模型失配）两部分，并据此触发针对性干预：高偶然不确定性触发观测恢复，高认知不确定性则调节控制动作。实验表明，该方法在机器人操作任务中显著提升了成功率，并在自适应感知任务中大幅降低了计算开销，证明了区分不确定性类型对提升系统鲁棒性和效率的重要性。 \
**Motivation**：现有系统因无法区分观测噪声与模型失配导致纠错措施针对性不足，甚至可能降低闭环性能。 \
**Method**：提出一种轻量化框架，利用马氏密度模型和前向动力学集成分别估计偶然与认知不确定性，并实施类型特定的干预策略。 \
**Result**：在机器人操作中将成功率从 59.4% 提升至 80.4%，在目标跟踪任务中减少了 58.2% 的计算量且精度损失极小。 \
**Conclusion**：通过解耦不确定性并实施针对性响应，可以显著增强机器人控制的鲁棒性并实现高效的自适应感知。
