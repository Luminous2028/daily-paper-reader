...
score: 8.0
evidence: 通过对并行去噪轨迹评分进行渐进式轨迹选择
tldr: 本研究探讨了视频扩散模型（如DiT）是否编码了物理真实性信号。研究发现，预训练DiT的中间层特征在不同噪声水平下均能区分物理上合理与不合理的视频。基于此，作者提出了一种推理时策略——渐进式轨迹选择。该方法利用在冻结特征上训练的轻量级物理验证器，在去噪过程中对多个并行轨迹进行评分并提前剪枝。在PhyGenBench上的实验证明，该方法在显著降低推理成本的同时，有效提升了视频的物理一致性。
selection_source: fresh_fetch
motivation: 探索预训练视频扩散模型内部是否包含可用于预测物理合理性的特征信号。
method: 提出渐进式轨迹选择策略，在推理过程中利用轻量级验证器对并行去噪路径进行实时评分与剪枝。
result: 在PhyGenBench上实现了与Best-of-K采样相当的物理一致性，且显著减少了所需的去噪步数和计算开销。
conclusion: 扩散模型的中间特征确实蕴含物理相关线索，通过推理时优化可有效提升生成视频的物理真实性。
---
