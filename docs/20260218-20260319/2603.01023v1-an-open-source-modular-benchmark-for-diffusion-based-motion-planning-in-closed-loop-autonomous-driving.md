...
title_zh: 闭环自动驾驶中基于扩散模型的运动规划开源模块化基准测试
...
tldr: 本研究针对基于扩散模型的运动规划器在闭环自动驾驶系统中评估不足的问题，提出了一个开源模块化基准测试框架。通过ONNX GraphSurgeon将庞大的扩散模型分解为三个独立模块，并在Autoware（ROS 2）中实现了原生C++的DPM-Solver++去噪循环。该框架支持运行时参数配置和去噪过程的可观测性，解决了传统单体部署的黑盒问题。实验证明，编码器缓存可降低3.2倍延迟，二阶求解器在低步数下显著提升了规划精度，为扩散模型从仿真到实车部署提供了路径。
selection_source: fresh_fetch
motivation: 现有的扩散模型规划器评估往往忽略了ROS 2通信延迟和实时调度约束，且单体ONNX部署缺乏参数调节的灵活性。
method: 利用ONNX GraphSurgeon将模型分解为独立模块，并在Autoware生产级堆栈中以原生C++重构去噪循环，实现模块化部署与实时观测。
result: "实验表明编码器缓存使延迟降低了3.2倍，且二阶DPM-Solver++在3步采样时比一阶方法减少了41%的终点位移误差。"
conclusion: 该开源基准测试框架打破了扩散模型部署的黑盒限制，为在真实自动驾驶系统中优化和验证扩散模型规划器提供了有效工具。
---

## 摘要
基于扩散模型的运动规划器在 nuPlan 等基准测试中取得了最先进的结果，但其在闭环生产级自动驾驶技术栈中的评估在很大程度上仍未得到探索。现有的评估忽略了 ROS 2 通信延迟和实时调度约束，而单体式 ONNX 部署在导出时会冻结所有求解器参数。我们提出了一个开源的模块化基准测试来填补这两个空白：利用 ONNX GraphSurgeon，我们将一个包含 18,398 个节点的单体扩散规划器分解为三个独立的可执行模块，并用原生 C++ 重新实现了 DPM-Solver++ 去噪循环。该系统作为 ROS 2 节点集成在 Autoware（部署在全球真实车辆上的开源自动驾驶栈）中，实现了无需模型重新编译的运行时可配置求解器参数，以及去噪过程的逐步可观测性，打破了单体部署的黑盒。与 CARLA 等独立模拟器中的评估不同，我们的基准测试在生产级技术栈内运行，并通过 AWSIM 闭环仿真进行了验证。通过在六种步数配置（N ∈ {3, 5, 7, 10, 15, 20}）下对 DPM-Solver++（一阶和二阶）和 DDIM 进行系统比较，我们表明编码器缓存可使延迟降低 3.2 倍，且在 N=3 时，二阶求解相比一阶可将 FDE 降低 41%。完整的代码库将开源发布，为从仿真基准测试到真实车辆部署提供直接路径。
