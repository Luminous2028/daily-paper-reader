...
score: 8.0
evidence: 利用潜在一致性信号进行可行性评分的扩散规划器
tldr: 针对扩散规划器在离线强化学习中因价值引导导致轨迹动力学不一致的问题，本文提出SAGE方法。通过自监督学习JEPA编码器和潜空间预测器，在推理阶段利用潜空间预测误差作为能量评分，对候选轨迹进行重新排序。该方法无需环境交互或重新训练策略，显著提升了扩散规划器在多种任务中的性能和鲁棒性。
selection_source: fresh_fetch
motivation: 扩散规划器在推理时容易选择高价值但动力学不一致的轨迹，导致执行失败。
method: 提出SAGE方法，利用JEPA架构学习潜空间一致性信号，通过预测误差（能量）对采样轨迹进行可行性评分和重排序。
result: 在运动、导航和操作等基准测试中，SAGE有效提高了现有扩散规划器的成功率和对动力学不一致性的鲁棒性。
conclusion: SAGE作为一种即插即用的推理增强方法，在不增加训练负担的情况下显著优化了扩散规划的决策质量。
---
