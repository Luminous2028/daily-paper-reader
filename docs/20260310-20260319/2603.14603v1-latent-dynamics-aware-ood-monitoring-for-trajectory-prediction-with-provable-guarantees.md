...
score: 8.0
evidence: 环境不确定性下轨迹预测的离群检测
tldr: 针对安全关键型CPS中的轨迹预测，本文提出了一种具有理论保证的分布外（OOD）监测框架。该研究将OOD检测建模为最快变化点检测问题，利用隐马尔可夫模型（HMM）捕捉分布内预测误差的潜在动力学特征，并结合累积最大均值差异（MMD）方法，在无需已知后验分布的情况下实现了低延迟、低误报的实时监测，显著增强了模型在复杂交通环境下的可靠性。
selection_source: fresh_fetch
motivation: 深度学习轨迹预测模型在面对环境不确定性或罕见交通行为等OOD场景时可靠性下降，且缺乏具有形式化保证的实时监测手段。
method: 将OOD监测表述为最快变化点检测问题，利用HMM建模分布内误差动态，并扩展了累积最大均值差异（MMD）方法以实现无监督检测。
result: 在三个真实驾驶数据集上的实验表明，该方法在减少检测延迟的同时，对重尾误差和未知的后验分布条件具有更强的鲁棒性。
conclusion: 本研究通过结合潜在动力学建模与统计检测理论，为轨迹预测系统提供了一种具备理论保证且高效的实时OOD监测方案。
---
