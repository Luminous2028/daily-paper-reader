...
score: 6.0
evidence: 在运动动力学中将物理推理与视觉综合解耦
tldr: 针对视频生成在复杂场景下难以兼顾视觉质量、物理一致性和可控性的挑战，本文提出 Motion Forcing 框架。该框架通过“点-形-色”分层范式，将物理推理与视觉合成显式解耦，依次建模稀疏几何锚点、动态深度图和高保真纹理。此外，引入掩码点恢复策略强制模型学习惯性等潜在物理规律。实验证明，该方法在自动驾驶及机器人等复杂任务中显著提升了生成的鲁棒性与物理真实感。
selection_source: fresh_fetch
motivation: 现有视频生成模型在处理碰撞或密集交通等复杂场景时，难以同时维持高视觉质量、严谨的物理一致性和精确的可控性。
method: 采用“点-形-色”解耦框架，将生成分解为稀疏几何锚点建模、动态深度图扩展和高保真纹理渲染，并结合掩码点恢复策略学习物理规律。
result: 在自动驾驶、物理和机器人基准测试中，Motion Forcing 在复杂场景下的表现显著优于现有最先进模型，保持了生成的三元平衡。
conclusion: 通过显式解耦物理推理与视觉合成，该框架为实现复杂动态场景下的鲁棒、可控视频生成提供了一种有效的层次化方案。
---

## 摘要
视频生成的最终目标是满足一个基本的三难困境：实现高视觉质量、保持严格的物理一致性以及实现精确的可控性。虽然最近的模型可以在简单、孤立的场景中保持这种平衡，但我们观察到，随着场景复杂性的增加（例如涉及碰撞或密集交通），这种平衡是脆弱的，且往往会崩溃。为了解决这一问题，我们引入了 Motion Forcing，这是一个旨在即使在复杂的生成任务中也能稳定这一三难困境的框架。我们的核心见解是通过分层的“点-形状-外观”（Point-Shape-Appearance）范式，将物理推理与视觉合成显式解耦。该方法将生成分解为可验证的阶段：将复杂的动力学建模为稀疏几何锚点（Point），将其扩展为显式解析 3D 几何的动态深度图（Shape），最后渲染高保真纹理（Appearance）。此外，为了培养鲁棒的物理理解，我们采用了掩码点恢复（Masked Point Recovery）策略。通过在训练期间随机掩码输入锚点并强制重建完整的动态深度，模型被迫超越被动的模式匹配，学习潜在的物理规律（例如惯性）以推断缺失的轨迹。在自动驾驶基准测试上的广泛实验表明，Motion Forcing 显著优于最先进的基准模型，在复杂场景中保持了三难困境的稳定性。在物理学和机器人学上的评估进一步证实了我们框架的通用性。
