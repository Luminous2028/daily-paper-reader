...
tldr: "针对多机器人系统在包含不可控动态代理（如行人）的共享环境中的冲突问题，本文提出了流感知多智能体路径规划（FA-MAPF）框架。该方法通过学习不可控代理的运动模式并将其整合到中心化MAPF算法中，旨在减少机器人与动态障碍物之间的冲突。实验证明，该方法在不降低任务效率的前提下，能将冲突减少高达55%，显著提升了共享环境下的路径规划质量。"
selection_source: fresh_fetch
motivation: 现有的多智能体路径规划研究主要关注延迟下的完备性，而忽视了利用环境中的动态代理运动信息来主动减少冲突。
method: 提出FA-MAPF框架，将学习到的不可控代理运动模式（流信息）集成到中心化MAPF算法中进行路径优化。
result: "在仿真和真实人类轨迹数据集上的实验显示，FA-MAPF在保持任务效率的同时，最高可减少55%的冲突。"
