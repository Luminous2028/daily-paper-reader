...
## 摘要
视频生成模型已被用作机器人策略，在给定任务描述和观测的条件下预测执行任务的未来状态。以往的研究忽略了它们高昂的计算开销和漫长的推理时间。为了应对这一挑战，我们提出了草稿-目标采样（Draft-and-Target Sampling），这是一种无需训练且能提高推理效率的视频生成策略新型扩散推理范式。我们通过在单个模型中利用两条互补的去噪轨迹，引入了一种自博弈去噪方法：草稿采样采用大步长以快速生成全局轨迹，而目标采样则采用小步长对其进行验证。为了进一步加速生成，我们引入了令牌分块（token chunking）和渐进式接受策略，以减少冗余计算。在三个基准测试上的实验表明，我们的方法可以实现高达 2.1 倍的加速，并在极小影响成功率的情况下提升了当前最先进方法的效率。我们的代码已开源。
