...
title_zh: FLUX：通过修正流与从静态到动态学习加速跨具身生成式导航策略
...
## 摘要
自主导航需要广泛的技能，从静态目标到达（goal-reaching）到动态社交穿梭（social traversal），但目前的评估在不同的协议中仍然是碎片化的。我们引入了 DynBench，这是一个具有物理有效人群模拟功能的动态导航基准。结合现有的静态协议，它支持对六项基础导航任务的全面评估。在此框架内，我们提出了 FLUX，这是首个基于流（flow-based）的统一导航策略。通过线性化概率流，FLUX 用直线轨迹取代了迭代去噪，相比之前的基于流的方法，单步推理效率提高了 47%，相比基于扩散的方法提高了 29%。遵循从静态到动态的课程学习，FLUX 最初建立几何先验，随后通过动态社交环境中的强化学习进行微调。这种方案不仅加强了社交感知导航，还通过随机动作分布捕捉恢复行为，增强了静态任务的鲁棒性。FLUX 在所有任务中均达到了最先进的性能，并在轮式、四足和人形平台上展示了无需任何微调的零样本从仿真到现实（sim-to-real）的迁移。
