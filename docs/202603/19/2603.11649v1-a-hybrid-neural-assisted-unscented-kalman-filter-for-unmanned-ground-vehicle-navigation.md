...
score: 6.0
evidence: 预测地面车辆导航的噪声不确定性
tldr: "本研究针对无人地面车辆导航中传统无迹卡尔曼滤波（UKF）难以应对动态噪声的问题，提出一种混合神经辅助估计框架。该框架利用深度神经网络从原始传感器数据中预测噪声协方差，并采用sim2real方法仅在仿真数据上训练。实验表明，该方法在多种车型和环境下均表现出更强的鲁棒性，定位精度提升了12.7%。"
selection_source: fresh_fetch
motivation: 传统的卡尔曼滤波器使用恒定的噪声协方差矩阵，难以适应复杂多变的现实动态环境。
method: 开发了一个深度神经网络，直接从原始惯性和GNSS测量值中预测UKF的预测和测量噪声不确定性，并采用纯仿真数据进行训练。
result: "在包含越野车、乘用车和移动机器人的三个不同数据集上，该方法比自适应模型化方法在定位精度上提升了12.7%。"
conclusion: 该混合估计框架为无人地面车辆导航提供了一种可扩展且更具鲁棒性的解决方案，证明了sim2real训练在实际应用中的有效性。
---
