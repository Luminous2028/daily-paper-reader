...
tldr: 本研究提出TrajLoom框架，旨在从视频中生成密集的未来轨迹。该方法通过网格锚点偏移编码减少位置偏差，利用VAE构建紧凑的时空潜空间，并采用流匹配技术在潜空间内生成未来轨迹。研究还引入了TrajLoomBench基准。实验表明，该方法将预测长度从24帧大幅提升至81帧，在保持运动真实感和稳定性的同时，有效支持了视频生成与编辑等下游任务。
selection_source: fresh_fetch
motivation: 密集的未来运动轨迹预测对于视频理解和可控生成至关重要，但长程且稳定的轨迹建模仍是一个挑战。
method: 提出包含网格锚点偏移编码、时空一致性VAE以及基于流匹配的轨迹生成器的TrajLoom框架。
result: 成功将轨迹预测长度从24帧扩展到81帧，并在运动真实感、稳定性和长程预测能力上超越了现有技术。
conclusion: TrajLoom为密集轨迹预测提供了高效且稳定的解决方案，并为视频生成和编辑任务提供了强有力的支持。
---
