...
title_zh: 针对紧急换道的交互感知预测性环境控制屏障函数
...
## 摘要
混合交通流中的安全性关键运动规划对自动驾驶车辆而言仍具挑战性，特别是当涉及主车（EV）与周围车辆（SV）之间的交互时。在稠密交通中，换道的可行性在很大程度上取决于周围车辆对主车运动的响应方式。本文提出了一种交互感知安全框架，将此类交互纳入基于控制屏障函数（CBF）的安全评估中。所提方法在有限时域内预测近未来的车辆位置，从而捕捉周围车辆的反应性行为，并将其嵌入到基于 CBF 的安全约束中。为了应对周围车辆响应模型中的不确定性，本文开发了一种鲁棒扩展方法，将模型失配视为有界扰动，并将在线不确定性估计纳入屏障条件中。与忽略周围车辆反应的经典环境 CBF 方法相比，所提方法为交互式交通场景提供了保守性更低且信息更丰富的安全表征，同时提高了对建模车辆行为不确定性的鲁棒性。
