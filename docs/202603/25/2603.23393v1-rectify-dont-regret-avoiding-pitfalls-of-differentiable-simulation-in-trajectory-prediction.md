...
title_zh: 纠正而非后悔：避免轨迹预测中可微仿真的陷阱
...
score: 7.0
evidence: 自动驾驶轨迹预测与分布外状态
tldr: 针对自动驾驶轨迹预测中开环模型易产生复合误差及微分闭环模拟器存在“捷径学习”导致非因果优化的缺陷，本文提出了一种分离式退避时界展开方法。通过切断模拟步骤间的计算图，强制模型从漂移状态中学习真实的纠错行为，而非利用未来信息修正过去。在nuScenes等数据集上的实验证明，该方法显著提升了模型的鲁棒性，大幅降低了碰撞率并改善了预测多样性。
selection_source: fresh_fetch
motivation: 现有的微分闭环模拟器因梯度回传导致未来信息泄露，使模型学会非因果地“后悔”过去而非真实地“纠正”错误。
method: 提出分离式退避时界展开方法，通过显式切断模拟步骤间的计算图，迫使模型在漂移状态下学习真实的反应式恢复策略。
result: "在nuScenes和DeepScenario数据集上，该方法相比微分闭环训练减少了高达33.24%的碰撞，相比开环基准减少了27.74%的碰撞。"
conclusion: 通过消除微分模拟中的非因果梯度泄露，模型能够学习到更稳健的纠错能力，显著提升了复杂环境下的自动驾驶安全性与预测质量。
---

## 摘要
当前的开环轨迹模型在现实世界的自动驾驶中面临挑战，因为微小的初始偏差往往会级联成累积误差，使智能体进入分布外状态。虽然全可微闭环仿真器试图解决这一问题，但它们受到捷径学习的影响：损失梯度通过诱导状态输入反向传播，无意中将未来的真值信息直接泄露到模型之前的预测中。模型利用这些信号人为地避免漂移，非因果地“后悔”过去的错误，而不是学习真正的反应式恢复。为解决这一问题，我们引入了一种分离式滚动时界展开（detached receding horizon rollout）。通过显式切断仿真步骤之间的计算图，模型能够从偏离状态中学习真正的恢复行为，迫使其“纠正”错误，而非非因果地优化过去的预测。在 nuScenes 和 DeepScenario 数据集上的广泛评估表明，我们的方法产生了更稳健的恢复策略，在高重规划频率下，与全可微闭环训练相比，目标碰撞减少了高达 33.24%。此外，与标准开环基准相比，我们的不可微框架在密集环境中将碰撞减少了高达 27.74%，同时提高了多模态预测的多样性和车道对齐度。
