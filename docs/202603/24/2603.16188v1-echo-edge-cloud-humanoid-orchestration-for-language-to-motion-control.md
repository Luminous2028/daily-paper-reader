...
tldr: 本研究提出ECHO，一个用于类人机器人语言驱动全身控制的边云协同框架。该框架在云端利用基于扩散模型的生成器将自然语言指令转化为机器人原生的38维运动序列，并在边缘端通过强化学习追踪器实现闭环执行。ECHO消除了推理时的重定向需求，并结合教师-学生蒸馏与跌倒恢复机制，在Unitree G1机器人上实现了无需微调的稳定运动控制，显著提升了类人机器人的交互能力。
selection_source: fresh_fetch
motivation: 旨在解决类人机器人从自然语言指令到复杂全身运动控制的实时性、准确性以及从仿真到现实迁移的鲁棒性挑战。
method: 采用云端扩散模型生成机器人原生运动表示，并配合边缘端基于强化学习的教师-学生蒸馏策略与跌倒恢复机制进行闭环追踪。
result: 在HumanML3D基准测试中取得了极高的生成质量（FID 0.029），并在Unitree G1真机上实现了多种文本指令的零样本稳定执行。
conclusion: ECHO证明了边云协同架构在类人机器人语言驱动控制中的有效性，为实现复杂且鲁棒的机器人具身智能交互提供了可行方案。
---
