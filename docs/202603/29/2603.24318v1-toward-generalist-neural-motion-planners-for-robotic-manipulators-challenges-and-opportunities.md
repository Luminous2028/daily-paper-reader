...
motivation: 现有的通用机器人策略在复杂环境中受限于低层运动规划的效率和泛化能力。
method: 通过对现有神经运动规划器进行系统性综述，分析其在处理高维配置空间和障碍物避障方面的表现。
result: 总结了神经运动规划器在推理速度和多模态处理上的优势，同时也指出了其在分布外泛化方面的显著短板。
conclusion: 提出了建立通用神经运动规划器的发展路径，旨在解决领域特定的挑战并提升机器人在非结构化环境中的适应性。
---
