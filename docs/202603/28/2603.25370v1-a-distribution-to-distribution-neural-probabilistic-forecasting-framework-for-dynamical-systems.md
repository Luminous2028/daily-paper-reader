...
## 摘要
概率预测通过将预测表示为概率分布而非确定性轨迹，为动力系统中的不确定性量化提供了一个原则性框架。然而，现有的预测方法，无论是基于物理的还是基于神经网络的，在根本上仍然是面向轨迹的：预测分布通常通过集成或采样获得，而不是作为动力学对象直接演化。本研究开发了一种分布到分布（D2D）神经概率预测框架，直接对预测分布进行操作。该框架围绕一个可替换的神经预测模块引入了分布编码和解码结构，利用核均值嵌入（kernel mean embeddings）来表示输入分布，并使用混合密度网络（mixture density networks）对输出预测分布进行参数化。这种设计使得预测不确定性能在一个统一的端到端神经架构中进行递归传播，且模型的训练和评估直接根据概率预测技巧进行。该框架在 Lorenz63 混沌动力系统上进行了演示。结果表明，D2D 模型捕捉到了非线性动力学下非平凡的分布演化，在无需显式集成模拟的情况下生成了高质量的概率预测，并且与简化的完美模型基准相比具有竞争力，在某些情况下甚至表现更优。这些发现指向了概率预测的一种新范式，即预测分布是直接学习和演化的，而不是通过基于集成的不确定性传播间接重构的。
