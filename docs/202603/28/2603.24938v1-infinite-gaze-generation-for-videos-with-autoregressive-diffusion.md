...
## 摘要
预测视频中的人类注视是推进场景理解和多模态交互的基础。虽然传统的显著性图提供了空间概率分布，而扫描路径提供了有序的注视点，但这两种抽象方式往往会丢失原始注视数据中细粒度的时序动态。此外，现有模型通常受限于短期窗口（约 3-5 秒），无法捕捉现实世界内容中固有的长程行为依赖关系。我们提出了一个用于在任意长度视频中进行无限时界原始注视预测的生成框架。通过利用自回归扩散模型，我们合成了具有连续空间坐标和高分辨率时间戳特征的注视轨迹。我们的模型以显著性感知的视觉潜空间为条件。定量和定性评估表明，我们的方法在长程时空准确性和轨迹真实性方面显著优于现有方法。
