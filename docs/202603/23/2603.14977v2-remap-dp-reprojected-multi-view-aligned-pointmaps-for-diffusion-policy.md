...
tldr: ReMAP-DP 针对通用机器人策略在 3D 空间感知方面的不足，提出了一种结合重投影多视图与像素对齐点图（PointMaps）的双流扩散策略框架。该方法通过透视重投影解决了几何畸变问题，并利用可学习的模态嵌入融合语义与几何特征，实现了高精度的 patch 级对齐。实验证明，该方法在 RoboTwin 2.0 和 ManiSkill 3 等仿真及现实任务中均显著优于 DP3 等基准，展现了极高的操作精度和数据效率。
selection_source: fresh_fetch
motivation: 现有的 2D 视觉策略缺乏 3D 空间意识，而现有的 3D 集成方法受限于点云的不规则性和正交渲染的几何畸变。
method: 提出一种双流扩散策略，通过透视重投影将多视图与像素对齐的点图结合，并利用模态嵌入融合语义特征与几何描述符。
result: "在 RoboTwin 2.0 上达到 59.3% 的成功率，在 ManiSkill 3 的堆叠任务中比 DP3 提升了 28%，并在现实世界中表现出强健的性能。"
conclusion: ReMAP-DP 通过协同多视图重投影与点图对齐，有效提升了机器人高精度操作任务的 3D 感知能力和操作成功率。
---
