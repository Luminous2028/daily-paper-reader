...
tldr: 本研究针对智能驾驶中的视觉风险目标识别（Vision-ROI）问题，指出传统确定性方法在复杂场景下因忽视不确定性而导致检测失效。为此，提出“共形风险管预测”（CRTP）框架，首次在时空维度统一建模风险不确定性，并提供覆盖率保证。通过引入支持多风险耦合的新数据集和评估指标，实验证明该方法能显著提升识别鲁棒性，减少误报，为安全驾驶提供可靠保障。
selection_source: fresh_fetch
motivation: 现有视觉风险目标识别方法缺乏对时空不确定性的建模，导致在复杂或模糊场景下容易出现检测延迟或预测不稳定的安全隐患。
method: 提出共形风险管预测（CRTP）框架，通过统一的时空建模提供具有覆盖率保证的校准风险评分和不确定性估计。
result: 在包含多风险耦合的新数据集上，该方法显著提升了识别鲁棒性，并有效减少了下游任务中的无效制动警报。
conclusion: CRTP框架通过量化时空不确定性，为智能驾驶系统提供了一种更安全、更可靠的风险目标识别方案。
---

## 摘要
我们研究了基于目标重要性的视觉风险目标识别（Vision-ROI），这是智能驾驶系统中危险检测的一项关键能力。现有方法通常做出确定性决策并忽略了不确定性，这可能导致安全关键型故障。具体而言，在模糊场景中，固定的决策阈值可能会导致风险检测过早或延迟，以及时间上的预测不稳定，特别是在具有多个交互风险的复杂场景中。尽管存在这些挑战，当前方法仍缺乏一个原则性的框架来跨越空间和时间联合建模风险不确定性。我们提出了符合性风险管预测（Conformal Risk Tube Prediction），这是一种统一的公式化方法，能够捕捉时空风险不确定性，为真实风险提供覆盖保证，并生成带有不确定性估计的校准风险评分。为了进行系统评估，我们提出了一个新的数据集和指标，用于探测具有多风险耦合效应的多样化场景配置，而现有数据集并不支持这些配置。我们系统地分析了影响不确定性估计的因素，包括场景变化、各风险类别行为以及感知误差传播。我们的方法相比先前的方法有了实质性的改进，增强了 Vision-ROI 的鲁棒性和下游性能，例如减少了干扰性制动警报。更多定性结果请访问我们的项目网页：https://hcis-lab.github.io/CRTP/
