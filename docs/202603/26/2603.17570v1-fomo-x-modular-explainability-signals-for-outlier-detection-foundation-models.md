...
title_zh: FoMo X：异常检测基础模型的模块化可解释性信号
...
tldr: 针对表格基础模型（如PFN）在异常检测中缺乏可解释性的问题，本文提出FoMo-X框架。该框架通过在预训练模型嵌入层添加轻量级辅助诊断头，利用离线模拟数据蒸馏出严重程度和不确定性信号。FoMo-X在保持零样本检测性能的同时，实现了高效、可解释的诊断，为安全关键领域的异常检测提供了可扩展的信任保障。
selection_source: fresh_fetch
motivation: 现有的异常检测基础模型虽具备强大的零样本预测能力，但其黑盒特性和高昂的解释成本限制了在安全关键场景中的应用。
method: 提出FoMo-X框架，通过在冻结的PFN骨干网络上附加辅助诊断头（严重程度头和不确定性头），并利用生成式模拟器进行离线训练。
result: 在ADBench等基准测试上的实验表明，FoMo-X能以极低的推理开销高保真地还原诊断信号，并提供校准后的置信度。
conclusion: FoMo-X成功弥合了基础模型性能与操作可解释性之间的差距，为实现可信、高效的零样本异常检测开辟了新路径。
---

## 摘要
表格基础模型，特别是先验数据拟合网络（PFNs），通过实现无需训练即可对新数据集进行无监督零样本自适应，彻底改变了异常检测（OD）。然而，尽管这些模型具有强大的预测能力，但它们通常作为不透明的黑盒运行，输出的标量异常得分缺乏安全关键型决策所需的运行上下文。现有的事后解释方法通常在实时部署中计算成本过高，或者无法捕捉零样本推理中固有的认知不确定性。在这项工作中，我们介绍了 FoMo-X，这是一个模块化框架，为异常检测基础模型配备了内在的、轻量级的诊断能力。我们利用了这样一个见解：预训练 PFN 骨干网络的冻结嵌入已经编码了丰富的、受上下文约束的关系信息。FoMo-X 在这些嵌入上附加了辅助诊断头，并使用与骨干网络相同的生成模拟器先验进行离线训练。这使我们能够将计算昂贵的属性（例如基于蒙特卡洛 Dropout 的认知不确定性）蒸馏为确定性的单次推理。我们用两个新的头来实例化 FoMo-X：一个将偏差离散化为可解释风险层级的严重程度头（Severity Head），以及一个提供校准置信度度量的不确定性头（Uncertainty Head）。在合成和真实世界基准（ADBench）上的广泛评估表明，FoMo-X 以高保真度和微乎其微的推理开销恢复了地面真值诊断信号。通过弥合基础模型性能与运行可解释性之间的差距，FoMo-X 为实现可信的零样本异常检测提供了一条可扩展的路径。
