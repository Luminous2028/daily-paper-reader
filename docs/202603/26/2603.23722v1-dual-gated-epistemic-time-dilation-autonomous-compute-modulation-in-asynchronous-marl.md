...
tldr: "针对多智能体强化学习（MARL）在边缘设备部署时计算开销过大的问题，本文提出ETD-MAPPO算法。该方法引入双门控认知触发器，利用策略熵和双评论家价值差异来评估不确定性，使智能体能自主调节推理频率。通过SMDP建模和异步梯度掩码机制，该方法在保持高性能的同时，在Google足球等复杂任务中实现了约73.6%的计算量削减，并展现出涌现的时间角色分工特性。"
selection_source: fresh_fetch
motivation: 传统的同步MARL范式强制智能体逐帧进行深度神经网络推理，导致在资源受限的边缘设备上部署时能耗和热量开销过高。
method: 提出ETD-MAPPO，通过结合策略熵和双评论家价值分歧的触发机制，在SMDP框架下实现自主的异步计算调节与信用分配。
result: "在Google足球等复杂环境下，该方法比基准模型性能提升超60%，且在不损失表现的前提下显著降低了73.6%的计算开销。"
conclusion: 该研究证明了通过自主调节推理频率，智能体可以在复杂任务中实现计算效率与决策性能的平衡，并自发形成高效的时间角色分工。
---
