...
tldr: 本研究探讨了在MotionGPT3框架下，将扩散模型替换为修正流（Rectified Flow）目标函数对文本驱动人体动作生成的影响。通过在HumanML3D数据集上的受控实验，研究发现修正流在训练收敛速度、推理效率以及动作质量方面均优于传统的扩散模型。该工作证明了修正流在连续潜空间动作生成中的优势，为高效动作先验模型的设计提供了重要参考。
selection_source: fresh_fetch
motivation: 旨在验证在图像和音频领域表现优异的修正流目标函数是否能提升连续潜空间文本到动作生成的效率和质量。
method: 在保持模型架构和训练协议不变的情况下，对MotionGPT3中的扩散先验与修正流先验进行了严格的受控对比实验。
result: 实验表明修正流在更少的训练轮次内即可收敛，且在推理阶段以更少的采样步数实现了与扩散模型相当或更优的动作生成质量。
conclusion: 修正流目标函数的优势可成功迁移至动作生成领域，是构建高效、高质量文本驱动动作先验模型的关键选择。
---

## 摘要
最近的文本驱动运动生成方法涵盖了基于离散标记的方法和连续潜空间公式。MotionGPT3 是后一种范式的典范，它结合了学习到的连续运动潜空间和基于扩散的先验，用于文本条件下的合成。虽然在图像和音频生成中，修正流（rectified flow）目标函数相对于扩散模型已展现出良好的收敛性和推理时间特性，但这些优势是否能直接迁移到运动生成领域仍不明确。在这项工作中，我们在 MotionGPT3 框架内进行了一项受控实证研究，对比了扩散目标和修正流目标。通过保持模型架构、训练协议和评估设置不变，我们分离出了生成目标对训练动态、最终性能和推理效率的影响。在 HumanML3D 数据集上的实验表明，修正流在更少的训练轮次内即可收敛，更早达到强大的测试性能，并在相同条件下达到或超过了基于扩散的运动质量。此外，基于流的先验在广泛的推理步数范围内表现出稳定的行为，并能以更少的采样步数实现具有竞争力的质量，从而改善了效率与质量之间的权衡。总之，我们的结果表明，修正流目标函数的几个已知优势确实扩展到了连续潜空间文本到运动生成领域，强调了运动先验中训练目标选择的重要性。
