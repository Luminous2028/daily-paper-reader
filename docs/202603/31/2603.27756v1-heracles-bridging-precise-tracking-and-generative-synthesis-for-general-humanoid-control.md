...
tldr: 针对通用类人机器人控制中精确跟踪与灵活性难以兼顾的问题，本文提出 Heracles 框架。该框架引入一种状态条件扩散中间件，作为高层参考运动与底层物理跟踪器之间的桥梁。它能根据机器人实时状态自动调节：在状态对齐时保持高精度跟踪，在遭遇严重干扰时无缝切换为生成式合成器以产生拟人化的恢复轨迹。该方法显著提升了机器人在极端环境下的鲁棒性，实现了从刚性跟踪到生成式通用控制的跨越。
selection_source: fresh_fetch
motivation: 现有的类人机器人控制器多采用刚性参考跟踪模式，在面对剧烈环境扰动时缺乏人类般的生成式适应能力和鲁棒性。
method: 提出一种名为 Heracles 的状态条件扩散中间件，通过实时状态反馈在精确运动跟踪与生成式轨迹合成之间进行隐式切换。
result: 实验证明该框架在保持零样本跟踪精度的同时，极大增强了机器人应对极端扰动的恢复能力。
conclusion: 将生成式先验集成到控制回路中，成功将类人机器人控制从单一的跟踪范式提升为开放且鲁棒的生成式通用架构。
---
