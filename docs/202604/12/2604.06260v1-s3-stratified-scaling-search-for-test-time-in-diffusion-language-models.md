...
method: 提出S³方法，在去噪的每个步骤扩展多个候选轨迹，利用验证器评估并选择性重采样潜力候选者，同时保持搜索前沿的多样性。
...
## 摘要
推理阶段缩放（Test-time scaling）研究在不进行额外训练的情况下，固定的扩散语言模型（DLM）在给予更多推理计算资源时，是否能生成更好的输出。然而，朴素的 $K$ 选 1（best-of-$K$）采样存在根本性的局限，因为它重复地从同一个基础扩散分布中抽取样本，而该分布的高概率区域往往与高质量输出不一致。我们提出了 $S^3$（分层缩放搜索），这是一种经典的验证器引导搜索方法，它通过在去噪过程中（而非仅在最终输出阶段）重新分配计算资源来改进生成。在每个去噪步骤中，$S^3$ 扩展多个候选轨迹，使用轻量级的无参考验证器对其进行评估，并在保持搜索前沿多样性的同时，选择性地对有潜力的候选者进行重采样。该过程有效地近似了一个奖励倾斜（reward-tilted）的采样分布，该分布倾向于高质量输出，同时保持锚定在模型先验上。在 MATH-500、GSM8K、ARC-Challenge 和 TruthfulQA 上使用 LLaDA-8B-Instruct 进行的实验表明，$S^3$ 在各个基准测试中一致地提升了性能，在数学推理任务上取得了最大的增益，同时保持底层模型和解码方案不变。这些结果表明，对去噪轨迹进行经典搜索为扩散语言模型（DLMs）的推理阶段缩放提供了一种实用的机制。
