...
conclusion: 通过优化噪声调度和训练策略，有效提升了扩散模型在处理高精度时空动力学模拟任务时的性能与效率。
---

## 摘要
条件扩散模型是模拟复杂时空动力学的强大代理模型，但在高精度任务中，其准确性往往难以达到确定性神经仿真器的水平。在这项工作中，我们解决了自回归 PDE 扩散模型的两个关键局限性：次优的单步准确率以及展开训练（unrolled training）极高的计算成本。首先，我们刻画了噪声调度、重建误差降低率与扩散曝光偏差（exposure bias）之间的关系，证明了标准调度会导致次优的重建误差。利用这一见解，我们提出了一个“自适应噪声调度”框架，通过动态约束模型的曝光偏差来最小化推理重建误差。我们进一步表明，这种优化的调度支持一种快速的“代理展开训练”方法，能够在无需完整马尔可夫链采样成本的情况下稳定长期推演（rollouts）。在包括受迫纳维-斯托克斯方程、Kuramoto-Sivashinsky 方程和跨声速流在内的多种基准测试中，所提出的两种方法在短期准确性和长期稳定性方面均显著优于扩散模型和确定性基准模型。
