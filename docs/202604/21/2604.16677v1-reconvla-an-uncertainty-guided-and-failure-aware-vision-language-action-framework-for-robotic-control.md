...
title_zh: ReconVLA：一种用于机器人控制的不确定性引导且具备故障感知的视觉-语言-动作框架
...
score: 7.0
evidence: 不确定性引导的控制以及动作预测的校准不确定性估计
...
## 摘要
视觉-语言-动作 (VLA) 模型已成为通用的机器人控制器，能够将视觉观测和自然语言指令映射为连续的动作序列。然而，VLA 模型对其动作预测缺乏经过校准的置信度度量，从而限制了它们在需要预见不确定性和故障的现实环境中的可靠性。为了解决这一问题，我们提出了 ReconVLA，这是一种可靠的符合预测 (conformal) 模型，能够产生不确定性引导且具备故障感知的控制信号。具体而言，我们的方法直接将符合预测 (conformal prediction) 应用于预训练 VLA 策略的动作标记 (action token) 输出，从而产生与执行质量和任务成功率相关的校准不确定性估计。此外，我们将符合预测扩展到机器人状态空间，以便在故障发生前检测离群值或不安全状态，提供了一种简单而有效的故障检测机制，作为动作级不确定性的补充。我们在仿真和真实机器人实验中，针对多种操作任务对 ReconVLA 进行了评估。结果表明，经过符合化处理的动作预测在无需重新训练或修改底层 VLA 的情况下，能够持续改善故障预见能力，减少灾难性错误，并提供经过校准的置信度度量。
