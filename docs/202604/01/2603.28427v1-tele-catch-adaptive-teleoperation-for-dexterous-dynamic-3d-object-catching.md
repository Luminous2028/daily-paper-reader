...
tldr: 本研究提出Tele-Catch框架，旨在解决灵巧手在动态物体抓取任务中因时机和姿态误差导致的遥操作失败问题。该框架通过共享自主机制，利用DAIM机制将人类手套信号融合进扩散策略去噪过程，并结合DP-U3R模块从点云中提取几何特征以增强决策。实验证明，该系统在多种灵巧手和未知物体上均表现出极高的抓取准确率、鲁棒性及泛化能力。
selection_source: fresh_fetch
motivation: 传统的遥操作在处理动态抓取任务时，常因人类操作的时延、姿态和力度误差而失败，因此需要结合人类输入与自主策略的共享自主方案。
method: 提出了包含DAIM动态感知自适应集成机制和DP-U3R几何感知扩散策略的Tele-Catch框架，实现了遥操作信号与自主策略的深度融合。
result: 实验结果显示，Tele-Catch显著提升了动态抓取的成功率和鲁棒性，并在不同灵巧手硬件及未见过的物体类别上展现出一致的性能增益。
conclusion: Tele-Catch通过自适应融合人类经验与几何感知策略，为灵巧手处理复杂的动态交互任务提供了一种高效且通用的解决方案。
---
