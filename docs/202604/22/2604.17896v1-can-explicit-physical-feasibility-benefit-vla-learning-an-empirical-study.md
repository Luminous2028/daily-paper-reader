...
tldr: 本研究探讨了显式物理可行性监督对视觉-语言-动作（VLA）模型学习的影响。针对现有VLA模型仅通过模仿学习隐式获取物理约束的问题，本文提出了一种基于几何的可行性目标函数，并将其整合进扩散式VLA策略的训练中。通过避障操作任务的系统评估，结果证明该方法不仅提升了物理可靠性和任务成功率，还在低数据量下表现出更高的学习效率，证明了显式物理信号在构建可靠机器人策略中的重要价值。
selection_source: fresh_fetch
motivation: 现有的VLA模型通常缺乏对障碍物规避等硬性物理约束的显式监督，导致其难以准确理解复杂的几何结构。
method: 提出一种简单的几何接地可行性目标，并将其作为辅助监督信号集成到基于扩散的VLA策略训练中。
result: 实验结果显示，增加可行性监督显著提高了物理可靠性和任务性能，并提升了模型在数据稀缺情况下的学习效率。
conclusion: 显式物理可行性信号是模仿学习的有效补充，能够助力开发出更具鲁棒性和可靠性的VLA策略。
---
