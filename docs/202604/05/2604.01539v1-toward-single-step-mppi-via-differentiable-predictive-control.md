...
score: 6.0
evidence: 基于采样的路径积分控制方法
tldr: 本研究针对模型预测路径积分（MPPI）在长时域任务中计算开销大和采样效率低的问题，提出了Step-MPPI框架。该方法通过神经网络参数化MPPI的提议分布，并利用可微预测控制在长时域内进行自监督训练。Step-MPPI在保持单步前瞻的毫秒级低延迟的同时，实现了多步优化的远见，有效解决了高维和长控制时域下的控制难题。
selection_source: fresh_fetch
motivation: 传统MPPI的计算成本随预测时域增加而剧增，且采样协方差难以手动调优，限制了其在复杂实时任务中的应用。
method: 提出Step-MPPI框架，利用神经网络学习采样分布，并通过结合MPC代价、约束惩罚和最大熵正则化的长时域自监督学习进行训练。
result: 在多个高维及长时域挑战性任务中，该方法以单步前瞻的毫秒级延迟达到了多步优化器的性能水平。
conclusion: Step-MPPI通过将长时域目标嵌入神经分布策略，成功平衡了控制的前瞻性与实时性，为高效采样控制提供了新思路。
---
