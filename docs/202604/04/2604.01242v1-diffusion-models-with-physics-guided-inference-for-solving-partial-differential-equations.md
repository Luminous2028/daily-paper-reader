...
tldr: 本文提出一种物理引导推理的扩散模型用于求解偏微分方程（PDE），解决了传统物理信息训练方法泛化性受限的问题。该方法在标准数据驱动训练后，仅在推理阶段通过PDE残差能量函数、高斯平滑和边界约束引导逆向扩散过程。实验证明，该框架作为一种无需重新训练的通用随机求解器，在泊松、扩散和Burgers方程中均表现出高精度和强鲁棒性。
selection_source: fresh_fetch
motivation: 现有的物理信息扩散模型因在训练阶段耦合特定方程而缺乏泛化性，限制了其在不同物理场景下的灵活应用。
method: 采用标准数据驱动训练扩散模型，并在推理阶段引入PDE残差引导、高斯平滑和显式边界约束来确保物理一致性。
result: 在多种经典PDE上的数值实验验证了该方法具有优异的收敛性、高精度以及无需重新训练即可处理变系数问题的泛化能力。
conclusion: 该研究为PDE求解提供了一个统一的扩散模型框架，展示了其作为传统数值求解器和物理信息神经网络替代方案的巨大潜力。
---
