...
tldr: 针对多步生成式策略在机器人控制中推理速度慢的问题，本文提出了一种原生单步生成策略框架。通过漂移策略（DBP）将迭代优化过程内化至模型参数，并结合漂移策略优化（DBPO）实现稳定的在线强化学习。该方法在保持多模态建模能力的同时，推理速度提升达100倍，支持105.2Hz的高频闭环控制。
selection_source: fresh_fetch
motivation: 现有的多步扩散策略因推理时需多次迭代而导致计算开销巨大，难以满足高频闭环控制和在线强化学习的实时性需求。
method: 提出基于漂移的策略（DBP）利用固定点漂移目标实现单步生成，并开发DBPO框架以支持预训练模型在在线环境下的稳定更新。
result: 实验表明DBP在性能上媲美或超越多步扩散策略，推理速度提升百倍，并在真实双臂机器人上实现了稳定且高效的实时控制。
conclusion: 该框架成功解决了生成式策略在实时机器人控制中的效率瓶颈，为实现高效、稳定的在线策略优化提供了创新的技术路径。
---
