...
tldr: DriveVA是一个创新的自动驾驶世界模型，旨在解决现有模型在未知场景和传感器配置下泛化能力不足的问题。它利用预训练的大规模视频生成模型（DiT架构）的运动动力学先验，通过共享潜空间联合解码未来视觉预测和动作序列。该方法实现了规划与场景演化的高度一致性，并在NAVSIM、nuScenes和Bench2drive等多个基准测试中展现了卓越的闭环性能和跨域零样本泛化能力。
selection_source: fresh_fetch
motivation: 现有的自动驾驶世界模型在跨数据集泛化以及视频预测与轨迹规划的一致性方面存在局限。
method: 提出一种基于DiT解码器的世界模型，通过共享潜生成过程联合预测未来动作和视频，并结合视频延续策略提升长时一致性。
result: 在NAVSIM上达到90.9的PDM得分，并在nuScenes和Bench2drive的零样本测试中显著降低了L2误差和碰撞率。
conclusion: DriveVA证明了大规模视频生成模型具备强大的零样本驾驶潜力，能有效提升自动驾驶在复杂环境下的泛化与规划能力。
---
