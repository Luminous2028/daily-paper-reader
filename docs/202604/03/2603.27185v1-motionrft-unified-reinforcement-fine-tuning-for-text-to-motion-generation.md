...
tldr: 针对文本生成动作模型在语义一致性和真实感方面的不足，本文提出MotionRFT强化微调框架。该框架包含多维度奖励模型MotionReward，通过共享语义空间统一异构动作表示，并利用EasyTune实现高效的逐步微调。实验证明，该方法在显著降低显存消耗的同时，大幅提升了不同动作表示模型（如关节点和旋转矩阵）的生成质量和语义对齐度。
selection_source: fresh_fetch
motivation: 现有的文本生成动作模型难以兼顾多种动作表示和高层语义目标，且微调过程计算开销巨大。
method: 提出了包含多维度奖励模型MotionReward和逐步优化微调方法EasyTune的MotionRFT框架，实现了异构表示的统一学习与高效内存优化。
result: 在多个模型上显著降低了FID指标并提升了语义对齐度，同时相比现有方法大幅减少了显存占用。
conclusion: MotionRFT为文本生成动作领域提供了一个通用、高效且能显著提升生成质量的强化微调方案。
---
