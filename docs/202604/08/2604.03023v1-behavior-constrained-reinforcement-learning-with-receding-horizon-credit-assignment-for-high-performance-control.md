...
title_zh: 具有退避时界信用分配的行为约束强化学习，用于高性能控制
...
score: 6.0
evidence: 用于轨迹级控制的收敛时界预测机制
tldr: 本研究针对机器人控制中高性能与专家行为一致性难以兼得的问题，提出了一种行为约束强化学习框架。该方法引入后退时界预测机制以提供前瞻性奖励，并利用参考轨迹对策略进行条件化，从而在超越演示数据性能的同时，显式控制对专家行为的偏离。在专业赛车模拟实验中，该方法不仅实现了顶尖的单圈速度，还保持了与职业车手高度一致的驾驶风格，证明了其在复杂动态控制任务中的有效性与可靠性。
selection_source: fresh_fetch
motivation: 旨在解决强化学习在追求高性能时往往偏离人类专家行为，而模仿学习又受限于演示数据质量且难以超越专家水平的矛盾。
method: 提出一种结合后退时界信用分配与参考轨迹条件化策略的行为约束强化学习框架，通过前瞻性奖励引导策略学习。
result: 在专业级赛车模拟中，该方法在保持与职业车手行为高度一致的同时，取得了优于基准方法的性能和单圈成绩。
conclusion: 该方法成功实现了高性能与行为一致性的平衡，为复杂控制系统提供了一种能够模拟人类决策的高效且可靠的替代方案。
---

## 摘要
在机器人领域，学习既能保持与专家行为一致又能实现高性能的控制策略是一项根本性挑战。强化学习能够发现高性能策略，但往往会偏离理想的人类行为；而模仿学习则受限于演示质量，且难以超越专家数据进行提升。我们提出了一种行为约束强化学习框架，该框架在超越演示性能的同时，能够显式地控制对专家行为的偏离。由于动态控制中与专家一致的行为本质上是轨迹级的，我们引入了一种退避时界预测机制，用于建模短期未来轨迹并在训练期间提供前瞻性奖励。为了考虑人类行为在扰动和环境变化下的自然变异性，我们进一步将策略以参考轨迹为条件，使其能够表示专家一致行为的分布，而非单一的确定性目标。在实验方面，我们在使用专业驾驶员数据的高保真赛车模拟中评估了该方法，该领域具有极端动力学和极窄性能边界的特点。学习到的策略在保持与专家驾驶行为高度一致的同时，实现了具有竞争力的单圈耗时，在性能和模仿质量上均优于基准方法。除了标准基准测试外，我们还在驾驶员在环模拟器中进行了基于人类的评估，结果表明，学习到的策略能够重现与顶级专业赛车手反馈一致的、依赖于赛车设置的驾驶特征。这些结果表明，我们的方法能够学习到既优且符合行为一致性的高性能控制策略，并可作为复杂控制系统中人类决策的可靠替代方案。
