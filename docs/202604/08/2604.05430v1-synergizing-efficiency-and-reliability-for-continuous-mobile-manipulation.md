...
## 摘要
人类能够无缝融合前瞻性规划与即时反馈，无需停顿即可执行连续的移动操作任务，从而兼顾高效率与高可靠性。在机器人上复制这种流畅且可靠的行为仍然具有根本性的挑战，这不仅是因为长程规划与实时反应性之间存在冲突，还因为在不确定环境中过度追求效率会损害可靠性：它会削弱稳定的感知能力和补偿潜力，同时增加意外接触的风险。在这项工作中，我们提出了一个统一的框架，旨在协同连续移动操作中的效率与可靠性。该框架包含一个可靠性感知轨迹规划器，它将可靠执行的关键要素嵌入到时空优化中，从而生成高效且具有可靠性保障的全局轨迹。该规划器与一个基于阶段的切换控制器耦合，后者在追求效率的全局轨迹跟踪与追求可靠性的任务误差补偿之间进行无缝切换。我们还研究了一种分层初始化方法，旨在应对复杂的长程规划问题并促进在线重规划。真实世界的评估表明，我们的方法能够在存在不确定性（如动态扰动、感知和控制误差）的情况下，高效且可靠地完成连续任务。此外，该框架可以推广到具有多种末端执行器约束的任务。与最先进的基准方法相比，我们的方法在始终保持最高效率的同时，将任务成功率提高了 26.67% 至 81.67%。全面的消融实验进一步验证了各组件的贡献。源代码将会开源。
