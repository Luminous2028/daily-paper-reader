...
## 摘要
通过交替方向乘子法（ADMM）进行的并行轨迹优化已成为长时界运动规划的一种可扩展方法。然而，现有框架通常基于预定义的固定结构将问题分解为并行子问题。这种结构刚性通常会导致在高度受限区域出现优化停滞，其中少数滞后的子问题会延迟全局收敛。一种自然的补救措施是在线自适应地对这些停滞段进行重切分。然而，决定何时、何地以及如何切分超出了基于规则的启发式方法的能力。为此，我们提出了 ATRS，这是一个将共享深度强化学习策略嵌入并行 ADMM 循环的新型框架。我们将这种自适应调整建模为多智能体共享策略马尔可夫决策过程，其中所有轨迹段都作为同质智能体并共享统一的神经策略网络。这种参数共享架构赋予了系统尺寸不变性，使其能够处理重切分过程中动态变化的段数，并泛化到任意轨迹长度。此外，我们的公式本质上支持对未知环境的零样本泛化，因为我们的网络仅依赖于数值求解器的内部状态，而非环境的几何特征。为了确保求解器的稳定性，一种基于置信度的选举机制在每一步仅选择最停滞的段进行重切分。广泛的仿真表明，ATRS 加速了收敛，迭代次数减少了高达 26.0%，计算时间减少了高达 19.1%。现实世界的实验进一步证实了其对大规模离线全局规划和每周期 35 毫秒内的实时机载重规划的适用性，且没有仿真到现实的性能退化。
