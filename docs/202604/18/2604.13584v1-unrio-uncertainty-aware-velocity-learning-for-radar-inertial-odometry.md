...
score: 6.0
evidence: 里程计中感知不确定性的速度学习与校准
tldr: 本研究提出UNRIO，一种不确定性感知的雷达惯性里程计系统。该系统直接从原始毫米波雷达IQ信号（4D频谱）中学习速度，而非依赖传统的手工点云处理。通过基于Transformer的架构预测速度及其不确定性，并将其与IMU数据在滑动窗口位姿图中融合。实验表明，UNRIO在多种室内运动场景下表现优异，尤其解决了传统方法在侧向运动中因点云稀疏导致的精度下降问题。
selection_source: fresh_fetch
motivation: 传统雷达里程计依赖手工设计的信号处理流程，容易丢失原始频谱中的关键信息且难以适应复杂环境。
method: 采用基于Transformer的GRT架构处理4D频谱，通过三阶段训练实现速度预测与不确定性校准，并利用位姿图融合雷达与IMU数据。
result: 在IQ1M数据集上，UNRIO在多数序列中取得了最低的相对位姿误差，特别是在侧向运动轨迹上显著优于传统数字信号处理基准。
conclusion: 直接利用原始雷达信号进行端到端的速度学习和不确定性估计，能有效提升雷达惯性里程计在极端运动下的鲁棒性和定位精度。
---

## 摘要
我们提出了 UNRIO，这是一种不确定性感知的雷达-惯性里程计系统，它直接从原始毫米波雷达 IQ 信号而非处理后的点云中估计自身速度。现有的雷达-惯性里程计方法依赖于手工设计的信号处理流水线，这些流水线会丢弃原始频谱中的潜在信息，并需要精细的参数调优。为了解决这一问题，我们提出了一种基于 GRT 架构的 Transformer 神经网络，该网络处理完整的四维（4-D）频谱立方体，以两种模式预测机体坐标系下的速度：直接线性速度估计和逐角度仓（per-anglebin）多普勒速度图。该网络分三个阶段进行训练：在 LiDAR 投影深度上进行几何预训练、速度或多普勒微调，以及通过负对数似然损失进行不确定性校准，使其能够在输出预测的同时产生不确定性估计。这些不确定性估计被传播到滑动窗口位姿图中，该图将雷达速度因子与 IMU 预积分测量值进行融合。我们在 IQ1M 数据集上对 UNRIO 进行了训练和评估，涵盖了多种室内环境，包括训练中未见过的向前和侧向运动模式。我们的方法在大多数序列上实现了最低的相对位姿误差，特别是在侧向运动轨迹上，相比经典的数字信号处理（DSP）基准方法具有显著优势，因为在这些场景下，稀疏的点云会使传统速度估计器的性能下降。
