...
tldr: 针对扩散策略在机器人操作中训练收敛慢和推理效率低的问题，本文提出VADF框架。该框架在训练阶段通过自适应损失网络（ALN）进行难样本挖掘，优先处理高损失区域以加速收敛；在推理阶段利用分层视觉任务分割器（HVTS）将任务分解，并根据子任务复杂度动态调整噪声步长和执行序列长度。VADF具有模型无关性，能显著提升训练效率和推理成功率，降低计算开销。
selection_source: fresh_fetch
motivation: 扩散策略因均匀采样导致的难负样本不平衡和缺乏样本难度感知，造成训练收敛缓慢及推理超时失败。
method: 提出由ALN（训练期难样本加权采样）和HVTS（推理期任务自适应分割与动态步长调整）组成的双重自适应框架。
result: 实验表明该框架显著减少了训练收敛步数，降低了推理计算开销，并大幅提升了早期任务成功率。
conclusion: VADF是一种通用的视觉驱动框架，通过在训练和推理中引入难度感知，有效解决了扩散策略的效率瓶颈。
---

## 摘要
扩散策略正逐渐成为机器人操作领域的主流，但由于均匀采样和缺乏对样本难度的感知，面临着难负样本类别不平衡的问题，导致训练收敛缓慢以及推理过程中频繁出现超时失败。我们提出了 VADF（视觉自适应扩散策略框架），这是一种视觉驱动的双重自适应框架，能够显著减少收敛步数并在推理中实现早期成功，其模型无关的设计使其能够无缝集成到任何扩散策略架构中。在训练期间，我们引入了自适应损失网络（ALN），这是一种基于 MLP 的轻量级损失预测器，可实时量化每步样本的难度。在难负样本挖掘的引导下，它执行加权采样以优先处理高损失区域，从而实现自适应权重更新和更快的收敛。在推理阶段，我们设计了分层视觉任务分割器（HVTS），它根据视觉输入将高层任务指令分解为多阶段的底层子指令。它通过为简单动作分配较短的噪声调度和较长的直接执行序列，为复杂动作分配较长的噪声步数和较短的执行序列，从而将动作序列自适应地分割为简单和复杂的子任务，进而大幅降低计算开销并显著提高早期成功率。
