...
tldr: 针对复杂多房间环境下的运动规划难题，本文提出了一种弱监督物理信息神经运动规划框架H-NTFields。该方法通过结合稀疏路线图提供的全局拓扑约束与物理信息偏微分方程（PDE）的局部几何正则化，克服了现有自监督物理信息方法易陷入局部最优且难以保证全局一致性的缺陷。在Gibson环境和真实机器人平台上的实验证明，该方法在保持快速推理的同时，显著提升了规划的鲁棒性和全局搜索能力。
selection_source: fresh_fetch
motivation: 现有的物理信息神经运动规划方法在复杂多房间场景中难以处理局部最优解，且缺乏全局一致性。
method: 提出H-NTFields框架，利用稀疏路线图作为弱监督信号提供全局拓扑锚点，并结合PDE损失函数确保局部几何保真度。
result: 在18个Gibson环境及真实机器人上的实验表明，该方法显著提高了规划的鲁棒性，并实现了连续价值表示下的快速推理。
conclusion: 通过结合稀疏路线图的全局引导与物理信息的局部约束，H-NTFields为复杂环境下的高效、鲁棒运动规划提供了一种有效的弱监督学习方案。
---

## 摘要
运动规划问题要求在高维、杂乱的空间中寻找起始与目标配置之间的无碰撞路径。最近基于学习的方法提供了极具前景的解决方案，其中自监督物理信息方法（如神经时间场 NTFields）通过求解程函方程（Eikonal equation）在无需专家演示的情况下学习价值函数。然而，现有的物理信息方法在复杂的多房间环境中难以扩展，在这些环境中，单纯增加样本数量无法解决局部极小值问题或保证全局一致性。我们提出了分层神经时间场（H-NTFields），这是一个将来自稀疏路线图的弱监督与物理信息偏微分方程（PDE）正则化相结合的弱监督框架。该路线图通过行程时间的上下界提供全局拓扑锚点，而 PDE 损失则确保局部几何保真度和障碍物感知传播。在 18 个 Gibson 环境和真实机器人平台上的实验表明，H-NTFields 相比之前的物理信息方法显著提高了鲁棒性，同时通过连续价值表示实现了快速的摊销推理。
