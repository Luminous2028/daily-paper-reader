...
tldr: 针对自动驾驶在交互式交通中缺乏安全关键长尾数据的问题，本文提出 E2 闭环框架，将对抗性场景生成从静态验证转变为自适应进化课程。该方法通过在逆时 SDE 先验上进行输运正则化稀疏控制，并结合拓扑驱动的支持选择与锚定技术，实现了高效且真实的对抗场景合成。实验表明，E2 在 nuScenes 和 nuPlan 数据集上显著提升了故障发现率，并通过闭环微调增强了策略的鲁棒性。
selection_source: fresh_fetch
motivation: 静态数据集缺乏安全关键的长尾事件，且现有对抗性测试多为开环或事后分析，难以将发现的故障有效反馈至训练过程。
method: 提出 E2 框架，利用拓扑驱动的支持选择和锚定技术，在逆时 SDE 先验上通过输运正则化稀疏控制生成对抗场景。
result: "在 nuScenes 和 nuPlan 数据集上，碰撞故障发现率分别提升了 9.01% 和 21.43%，同时保持了极高的场景真实性。"
conclusion: 将对抗性评估转化为进化课程学习，能有效挖掘边界案例并显著提升自动驾驶策略在复杂交互环境下的鲁棒性。
---
