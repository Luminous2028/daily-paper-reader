...
tldr: 本研究针对分子动力学（MD）轨迹生成中数据稀缺和高维建模难的问题，提出了一种基于结构预训练的新型框架。该方法首先在大规模构象数据集上训练扩散生成模型，再通过插值模块在MD轨迹数据上实现时间一致性。通过将复杂任务分解为结构生成与时间对齐，该模型在小分子及蛋白质系统上表现优异，显著提升了几何、动力学和能量预测的准确性。
selection_source: fresh_fetch
motivation: 针对分子动力学轨迹数据稀缺及高维分布建模复杂的挑战，探索利用大规模静态结构数据辅助轨迹生成。
method: 提出一种两阶段框架，先利用扩散模型进行结构预训练，再引入插值模块对生成的结构进行时间对齐以确保轨迹连贯性。
result: 在QM9、DRUGS及蛋白质系统上的实验表明，该方法在几何、动力学和能量指标上均取得了显著的精度提升。
conclusion: 通过结构预训练与时间对齐的解耦策略，该框架有效缓解了MD数据不足的问题，为生成高质量分子动力学轨迹提供了新思路。
---

## 摘要
使用深度生成模型生成分子动力学（MD）轨迹已引起越来越多的关注，但由于 MD 数据的有限可用性以及建模高维 MD 分布所涉及的复杂性，这仍然具有内在的挑战性。为了克服这些挑战，我们提出了一个利用结构预训练进行 MD 轨迹生成的创新框架。具体而言，我们首先在大规模构象数据集上训练一个基于扩散的结构生成模型，并在此基础上引入一个在 MD 轨迹数据上训练的插值器模块，旨在增强生成结构之间的时间一致性。我们的方法有效地利用了丰富的结构数据来缓解 MD 轨迹数据的稀缺性，并将复杂的 MD 建模任务有效地分解为两个可处理的子问题：结构生成和时间对齐。我们在 QM9 和 DRUGS 小分子数据集上，针对无条件生成、前向模拟和插值任务对我们的方法进行了全面评估，并进一步将我们的框架和分析扩展到四肽和蛋白质单体系统。实验结果证实，我们的方法在生成化学上真实的 MD 轨迹方面表现出色，几何、动力学和能量测量准确性的显著提升证明了这一点。
