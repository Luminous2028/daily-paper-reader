...
title_zh: HiFlow：基于流匹配的无分词跨尺度自回归策略学习
...
tldr: 本研究针对机器人视觉运动策略学习，提出了HiFlow框架。针对现有自回归方法依赖离散分词器导致的量化误差和训练复杂性问题，HiFlow采用无分词器的粗到细自回归架构，直接在连续动作空间操作。通过时间池化构建多尺度目标并结合流匹配进行端到端训练，在多个仿真和真实环境实验中表现优于扩散模型和基于分词的自回归策略。
selection_source: fresh_fetch
motivation: 现有的自回归策略依赖离散分词器处理低维机器人动作，导致了不必要的量化误差和复杂的多阶段训练流程。
method: 提出一种基于流匹配的无分词器多尺度自回归策略，通过时间池化生成连续动作摘要并进行逐级细化。
result: 在MimicGen、RoboTwin 2.0及真实世界环境的实验中，HiFlow的性能一致优于扩散模型和基于分词的自回归基准。
conclusion: HiFlow证明了在连续动作空间直接进行多尺度自回归建模的有效性，简化了训练流程并提升了策略精度。
---

## 摘要
由粗到细的自回归建模最近在视觉运动策略学习中展现出强大的潜力，它结合了自回归方法的推理效率与基于扩散策略的全局轨迹连贯性。然而，现有方法依赖于离散动作分词器，将连续动作序列映射到码本索引，这种设计继承自图像生成领域，在处理高维像素数据时，学习压缩是必要的。我们观察到机器人动作本质上是低维连续向量，对此类数据进行分词会引入不必要的量化误差和多阶段训练流程。在这项工作中，我们提出了分层流策略（HiFlow），这是一种直接在原始连续动作上运行的无分词由粗到细自回归策略。HiFlow 通过简单的时域池化，从每个动作块中构建多尺度连续动作目标。具体而言，它对连续的动作窗口取平均值以生成粗略摘要，并在更精细的时间分辨率下进行细化。整个模型在单阶段内进行端到端训练，消除了对独立分词器的需求。在 MimicGen、RoboTwin 2.0 和真实环境中的实验表明，HiFlow 始终优于现有方法，包括基于扩散和基于分词的自回归策略。
