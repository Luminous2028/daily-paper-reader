...
title_zh: 从独立扩散到相关扩散：利用概率计算机进行的广义生成建模
...
tldr: 本研究提出一种广义扩散模型框架，将传统独立噪声替换为包含相互作用结构的MCMC动力学，实现相关扩散。该方法利用伊辛耦合捕捉空间相关性，并能高效映射到概率计算机硬件上。实验证明，该方法在物理系统建模中优于独立扩散，且在采样吞吐量和能效上显著优于GPU，为结构化概率采样驱动的生成模型开辟了新方向。
selection_source: fresh_fetch
motivation: 传统扩散模型依赖独立噪声采样，忽略了随机过程中的结构化相关性及专用概率硬件的加速潜力。
method: 将扩散过程中的独立噪声替换为包含相互作用结构的MCMC动力学，并利用基于概率位的硬件进行加速。
result: 在伊辛模型和自旋玻璃模型上，相关扩散生成的样本更接近真实分布，且硬件能效比GPU高出数个数量级。
conclusion: 该框架证明了概率计算机能通过结构化采样赋能新型扩散算法，显著提升生成模型的性能与计算效率。
---

## 摘要
扩散模型已成为深度学习中生成任务的强大框架。它们将生成建模分解为两个计算原语：确定性神经网络评估和随机采样。目前的实现通常将大部分计算集中在神经网络中，但扩散作为一个框架，允许对随机转移核进行更广泛的选择。在本文中，我们通过将独立噪声注入替换为包含已知相互作用结构的马尔可夫链蒙特卡罗（MCMC）动力学，推广了随机采样组件。当耦合项设为零时，标准独立扩散可作为特例恢复。通过在扩散动力学中显式引入伊辛（Ising）耦合，加噪和去噪过程利用了代表目标系统的空间相关性。由此产生的框架自然地映射到由概率比特（p-bits）构建的概率计算机（p-computers）上，与 GPU 相比，这些计算机在采样吞吐量和能效方面具有数量级的优势。我们在二维铁磁伊辛模型和三维 Edwards-Anderson 自旋玻璃的平衡态上演示了该方法，结果表明，与独立扩散相比，相关扩散产生的样本与 MCMC 参考分布的一致性更高。更广泛地说，该框架表明概率计算机可以实现新型扩散算法，利用结构化概率采样进行生成建模。
