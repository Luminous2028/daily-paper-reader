...
tldr: 针对现代世界模型过于复杂、难以显式描述动力学的问题，本文提出了一种基于在线黎曼优化的惩罚预测控制（PPC）框架。该方法利用模拟器将可行性流形压缩为基于分数的条件密度，为动作空间赋予黎曼几何结构以引导规划器优化。研究证明了安全性边界受分数估计误差和屏障曲率影响，且随上下文信息的丰富而提升。实验表明，该方法在动态导航任务中显著优于传统模型，尤其在环境变化时表现更佳。
selection_source: fresh_fetch
motivation: 现代世界模型日益复杂，导致在安全关键任务中难以通过显式动力学描述进行有效的上下文控制。
method: 提出PPC框架，利用基于分数的条件密度构建动作空间的黎曼几何，并通过在线黎曼优化引导规划器。
result: 理论上给出了受分数估计误差和屏障曲率约束的安全性边界，仿真实验验证了其在动态导航中的优越性。
conclusion: 引入上下文信息的黎曼优化能有效提升黑盒模拟器下的控制安全性与收敛性，且对环境偏移具有更强的鲁棒性。
---
